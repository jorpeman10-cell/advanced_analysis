...
var DE_TEXTS = {
  ...
  lintCheckingDuplicatesProgress: "Duplikate werden \xFCberpr\xFCft: Stapel {current}/{total}...",
...
var FR_TEXTS = {
  ...
  lintCheckingDuplicatesProgress: "V\xE9rification des doublons : lot {current}/{total}...",
  ...
  display() {
    ...
    new import_obsidian3.Setting(containerEl).setName(this.getText("fetchModelsName")).setDesc(this.getText("fetchModelsDesc")).addButton((button) => button.setButtonText(this.getText("fetchModelsButton")).onClick(async () => {
      ...
      try {
        ...
        new import_obsidian3.Notice(this.getText(`fetchError${category}`), NOTICE_ERROR);
        this.tempSettings.useCustomModel = true;
        this.tempSettings.availableModels = [];
        this.display();
      }
...
(Wiki has ${allPages.length} pages total; showing first ${MAX_PAGES}. See index.md for the full list.)`;
  ...
  async mergePage(info, pageType, sourceFile, existingContent, extraPagePaths, path) {
    ...
    try {
      ...
      const prompt = mergePrompt.replace("{{existing_body}}", existingBody).replace("{{new_source}}", sourceFile.basename).replace("{{entity_summary}}", info.summary).replace("{{concept_summary}}", info.summary).replace("{{mentions}}", truncateMentions(info.mentions_in_source)).replace("{{related_entities}}", ((_a = info.related_entities) == null ? void 0 : _a.join(", ")) || "").replace("{{related_concepts}}", ((_b = info.related_concepts) == null ? void 0 : _b.join(", ")) || "").replace("{{key_details}}", ((_c = info.mentions_in_source) == null ? void 0 : _c.slice(0, 2).join("; ")) || "").replace("{{existing_pages}}", await this.buildPagesListForPrompt(extraPagePaths));
      const finalPrompt = applySectionLabels(prompt, this.ctx.settings);
...
${cleanedBody}`;
  ...
  async appendToReviewedPage(info, sourceFile, existingContent, path) {
    ...
    try {
      const { frontmatter, body: existingBody } = mergeFrontmatter(existingContent, sourceFile.path);
      const prompt = PROMPTS.appendToReviewedPage.replace("{{existing_body}}", existingBody).replace("{{new_source}}", sourceFile.basename).replace("{{entity_summary}}", info.summary).replace("{{mentions}}", truncateMentions(info.mentions_in_source)).replace("{{key_details}}", ((_a = info.mentions_in_source) == null ? void 0 : _a.slice(0, 2).join("; ")) || "").replace("{{constraints}}", UNIVERSAL_LINK_CONSTRAINTS);
      const finalPrompt = applySectionLabels(prompt, this.ctx.settings);
      ...
      try {
        ...
        if (verifyCandidates.length > 0) {
          ...
          for (let i = 0; i < batches.length; i += concurrency) {
            ...
            stageNotice.setMessage(t.lintCheckingDuplicatesProgress.replace("{current}", progressLabel).replace("{total}", String(batches.length)));
